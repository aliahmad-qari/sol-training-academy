...
import {
  ...
  Award, BarChart2, AlertCircle, AlertTriangle, X, Timer, Lock
} from "lucide-react";
...
function AssignmentCard({ assignment, submission, userId, onSubmit }) {
  const { countdown } = useDeadlineCountdown(assignment, userId);
  const isExpired = countdown?.expired;
                      ...
                      {quizAttempts.slice(0, 4).map((a, i) => (
                        <div key={i} className="flex items-center justify-between">
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-[10px] text-slate_mist">{new Date(a.created_date).toLocaleDateString("en-AU")}</p>
                          </div>
                        ...
                        {quizAttempts.map((attempt, i) => {
                          ...
                          return (
                            <tr key={i} className="border-b border-border/20 hover:bg-slate-50">
                              ...
                              <td className="px-4 py-3 text-xs text-slate_mist">{new Date(attempt.created_date).toLocaleDateString("en-AU")}</td>
                              <td className="px-4 py-3">
                                <div className="flex items-center gap-2">
                                  <div className="w-16 h-1.5 bg-slate-100 rounded-full overflow-hidden">
                        ...
                        <div key={`s-${i}`} className="px-5 py-3 flex items-center justify-between">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-[10px] text-slate_mist">Submitted {new Date(sub.created_date).toLocaleDateString("en-AU")}</p>
                            </div>
                          </div>
                          <div className="flex items-center gap-2">
                            {sub.status === "graded" ? (
